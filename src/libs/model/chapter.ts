...
// TODO: where and undefined.
export const toChapterList = (bookId: string | undefined) => {
  if (!bookId) {
    console.error('Id not defined!');
    return '/trainer';
  }
  return `/trainer/book/${bookId}/chapters`;
};
...
export const toChapter = (
  ...
  return `/trainer/book/${bookId}/${action}/${chapterId}`;
};
